"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
//ova komponenta nam sluzi da napravimo novi tag u
//html-u  kao tag my-apps .da usitnimo kod
var core_1 = require("@angular/core");
var client_1 = require("./client");
var router_1 = require("@angular/router");
var common_1 = require("@angular/common");
var client_service_1 = require("./client.service");
require("rxjs/add/operator/switchMap");
//pozivamo ,a ne importujemo klasu
var ClientUpdateComponent = (function () {
    function ClientUpdateComponent(clientService, route, location) {
        this.clientService = clientService;
        this.route = route;
        this.location = location;
    }
    ClientUpdateComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.route.params
            .switchMap(function (params) { return _this.clientService.getClient(+params['id']); })
            .subscribe(function (client) { return _this.client = client; });
    };
    ClientUpdateComponent.prototype.goBack = function () {
        this.location.back();
    };
    ClientUpdateComponent.prototype.updateClient = function () {
        var _this = this;
        this.clientService.updateClient(this.client)
            .then(function () { return _this.goBack(); });
    };
    return ClientUpdateComponent;
}());
__decorate([
    core_1.Input(),
    __metadata("design:type", client_1.Client)
], ClientUpdateComponent.prototype, "client", void 0);
ClientUpdateComponent = __decorate([
    core_1.Component({
        selector: 'client-update',
        templateUrl: './client-update.component.html',
        styleUrls: ['./client-update.component.css'],
    }),
    __metadata("design:paramtypes", [client_service_1.ClientService,
        router_1.ActivatedRoute,
        common_1.Location])
], ClientUpdateComponent);
exports.ClientUpdateComponent = ClientUpdateComponent;
//# sourceMappingURL=client-update.component.js.map